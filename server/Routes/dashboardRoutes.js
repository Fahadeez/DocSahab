...
module.exports = app => {
	...
	app.get('/api/selectAllTutors', async (req, res) => {
		...
		const query = User.find({ role: 'Tutor' }).sort({ _id: -1 }).limit(checkLim)
	...
	app.get('/api/selectTutor/:id', async (req, res) => {
	...
	app.post('/api/selectTutorsWithTheirName', async (req, res) => {
		const { limit } = req.query;
		const lim = parseInt(limit)
		const checkLim = lim === 0 ? 10 : lim
		...
		// const { gender, qualification, country, state, city } = filters;
		if (name && filters) {
			const { gender, qualification, country, city, state } = req.body.filters;
			let condition = { role: 'Tutor', firstName: name.toLowerCase() };
			...
			const query = User.find(condition).limit(checkLim)
			query.exec((err, users) => {
				if (err) {
					return res.send('Error in DB').status(400);
				}
			...
			const query2 = User.find({ role: 'Tutor', firstName: name.toLowerCase() }).limit(checkLim)
			query2.exec((err, users) => {
				if (err) {
					return res.send('Error in DB').status(400);
				}
				if (users) {
			...
			const query3 = User.find(condition).limit(checkLim)
			query3.exec((err, users) => {
				if (err) {
					return res.send('Error in DB').status(400);
				}
				if (users) {
			...
			return res.redirect('/api/selectAllTutors?limit=10')
			// const { limit } = req.query;
			// const lim = parseInt(limit)
